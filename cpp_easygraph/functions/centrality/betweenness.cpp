...
void betweenness_bfs_worker(
    ...
    while (front < back) {
        int u = q[front++];
        int du = dis[u];
        if (cutoff_int >= 0 && du > cutoff_int) break;
        St[cnt_St++] = u;
        for (int p = head[u]; p != -1; p = E[p].next) {
            int v = E[p].to;
            int new_dis = du + 1;
            if (cutoff_int >= 0 && new_dis > cutoff_int) continue;
    ...
    while (cnt_St > 0) {
        ...
        if (u != S) bc[u] += delta[u] + endpoints_;
    }
}

void betweenness_dijkstra_worker(
    const Graph_L& G_l, const int& S, std::vector<double>& bc, double cutoff,
    Segment_tree_zkw& segment_tree_zkw, std::vector<int>& dis, std::vector<int>& head_path,
    ...
    std::fill(dis.begin(), dis.end(), dis_inf);
    ...
    while (segment_tree_zkw.t[1] != dis_inf) {
        ...
        if (cutoff >= 0 && dis[u] > cutoff) continue;
        St[cnt_St++] = u;
        for (int p = head[u]; p != -1; p = E[p].next) {
            int v = E[p].to;
            int w = E[p].w;
...
#pragma omp parallel for schedule(dynamic)
        ...
        for (int i = 0; i < sources_list_len; i++) {
            ...
            std::vector<int>& q = queue_all[tid];
            std::vector<int>& stamp = stamp_all[tid];
            int& cur_stamp = cur_stamp_all[tid];
            if (use_weights) {
                betweenness_dijkstra_worker(
                    G_l, source_id, bc_local, cutoff_, segment_tree_zkw, dis, head_path,
                    St, count_path, delta, E_path, endpoints_
                );
            } else {
...
#pragma omp parallel for schedule(dynamic)
        ...
        for (int i = 1; i <= N; ++i) {
            ...
            if (use_weights) {
                betweenness_dijkstra_worker(
                    G_l, i, bc_local, cutoff_, segment_tree_zkw, dis, head_path,
                    St, count_path, delta, E_path, endpoints_
                );
            } else {
                betweenness_bfs_worker(
