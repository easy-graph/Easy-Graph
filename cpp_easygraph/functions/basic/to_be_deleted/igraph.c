 ...
 * is taken. Vertices with less than two neighbors require special treatment,   [note]
 ...
 * Edge directions and edge multiplicities are ignored. [note]
 *
 * </para><para>
 * Note that this measure is different from the global transitivity measure [note]
...
igraph_error_t igraph_transitivity_local_undirected(const igraph_t *graph, // [note]
        igraph_vector_t *res,
        const igraph_vs_t vids,
        igraph_transitivity_mode_t mode) {
 ...
 *   directed graphs. Note that the function does \em not work for [note]
 *   non-simple graphs.
 * \param res Pointer to an initialized vector, the result will be
 *   stored here. It will be resized as needed.
