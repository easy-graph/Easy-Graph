"""基准测试 – web-NotreDame 数据集 | igraph

运行示例:
  python test_enron_ig.py                        # 默认 (5 进程 × 3 次采样，含 1 次热身)
  python test_enron_ig.py --fast                 # 快速模式
  python test_enron_ig.py -o results_enron_ig.json
...
说明:
  ...
  - 若需固定线程数以公平对比多库，建议: OMP_NUM_THREADS=1 python test_enron_ig.py
"""
import os
import sys
import pyperf
...
if __name__ == "__main__":
    if '--worker' not in sys.argv:
        print(f"web-NotreDame: {G_ig.vcount()} nodes, {G_ig.ecount()} edges")
        _print_thread_info()
