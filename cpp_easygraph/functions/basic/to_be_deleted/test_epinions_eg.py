"""基准测试 – soc-Epinions1 数据集 | cpp_easygraph (DiGraph)

运行示例:
  python test_epinions_eg.py                        # 默认 (5 进程 × 3 次采样，含 1 次热身)
  python test_epinions_eg.py --fast                 # 快速模式
  python test_epinions_eg.py -o results_epinions_eg.json

说明:
  - 图构建在计时之外（模块级），每个 worker 进程独立初始化，保证进程隔离。
  - pyperf 自动处理热身、多次重复与统计（中位数 ± 标准差）。
  - 若需固定线程数以公平对比多库，建议: OMP_NUM_THREADS=1 python test_epinions_eg.py
"""
import os
import sys
import pyperf
...
DATA_PATH = os.path.join(os.path.dirname(__file__), 'soc-Epinions1.txt')
...
if __name__ == "__main__":
    if '--worker' not in sys.argv:
        print(f"soc-Epinions1: {G_cpp.number_of_nodes()} nodes, {G_cpp.number_of_edges()} edges")
        _print_thread_info()

    runner = pyperf.Runner(processes=5)
    runner.bench_func('CPP-EasyGraph: cpp_clustering (DiGraph)', bench_cpp_clustering)

# 原始 timeit_avg 版本已迁移到 pyperf，如需回退请参考 git history
            pass
