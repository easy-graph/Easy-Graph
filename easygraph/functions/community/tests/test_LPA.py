...
class TestLabelPropagationAlgorithms(unittest.TestCase):
    ...
    def test_slpa(self):
        self.assertEqual(eg.functions.community.SLPA(self.graph_single_node, T=5, r=0.01), {1: [42]})
        self.assertTrue(eg.functions.community.SLPA(self.graph_simple, T=10, r=0.1))
        self.assertTrue(eg.functions.community.SLPA(self.graph_disconnected, T=15, r=0.1))

    def test_hanp(self):
        self.assertEqual(eg.functions.community.HANP(self.graph_single_node, m=0.1, delta=0.05), {1: [42]})
        self.assertTrue(eg.functions.community.HANP(self.graph_simple, m=0.3, delta=0.1))
        self.assertTrue(eg.functions.community.HANP(self.graph_weighted, m=0.5, delta=0.2))

    def test_bmlpa(self):
        self.assertEqual(eg.functions.community.BMLPA(self.graph_single_node, p=0.1), {1: [42]})
