...
class Test_LINE(unittest.TestCase):
    ...
    def test_output_is_dict_with_correct_dim(self):
        model = eg.functions.graph_embedding.LINE(dimension=16, walk_length=10, walk_num=5, order=1)
    ...
    def test_output_as_matrix(self):
        model = eg.functions.graph_embedding.LINE(dimension=8, walk_length=5, walk_num=3, order=1)
        emb = model(self.graph, return_dict=False)
        self.assertEqual(emb.shape, (len(self.graph.nodes), 8))

    def test_output_with_order_2(self):
        model = eg.functions.graph_embedding.LINE(dimension=16, walk_length=10, walk_num=5, order=2)
        emb = model(self.graph)
        for vec in emb.values():
            self.assertEqual(len(vec), 16)

    def test_output_with_order_3_combination(self):
        model = eg.functions.graph_embedding.LINE(dimension=16, walk_length=10, walk_num=5, order=3)
        emb = model(self.graph)
        for vec in emb.values():
            self.assertEqual(len(vec), 16)
