...
from easygraph.functions.centrality.convert_matrix import *
__all__ = [
    "flow_matrix_row",
    "InverseLaplacian",
...
def flow_matrix_row(G, weight=None, dtype=float, solver='lu'):
    ...
    adj = G.adj
    for u, v in sorted(sorted((u, v)) for u, v, _ in G.edges):
        B = np.zeros(w, dtype=dtype)
        c = adj[u][v].get("weight", 1)
        B[u % w] = c
        B[v % w] = -c
...
def laplacian_sparse_matrix(G, nodelist=None, weight=None, dtype=None,
    ...
    import numpy as np
    import scipy.sparse
    A = to_scipy_sparse_matrix(G, nodelist=nodelist, weight=weight,
