...
class GCN(nn.Module):
    ...
    def forward(self, x, g):
        g.build_adj_gp(nparts=self.nparts)
        x = x[g.cache['gp_perm']]
        x = self.gcn1(x, g)
        x = F.relu(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.gcn2(x, g)
        x = x[g.cache['gp_inv_perm']]
        return x
...
def main():
    for backend_type, dataset_name in DATASETS:
        ...
        if backend_type == 'Coauthor':
            dataset = Coauthor(root=f'/root/autodl-tmp/data/Coauthor', name=dataset_name)
        elif backend_type == 'Planetoid':
            dataset = Planetoid(root=f'/root/autodl-tmp/data/{dataset_name}', name=dataset_name, transform=transform)
        elif backend_type == 'ogb':
            dataset = PygNodePropPredDataset(name=dataset_name, root='/root/autodl-tmp/data/OGB')
        else:
        ...
        print(f"🔥 总训练时间: {sum(All_total_train_time[1:])/(RR-1):.3f} 秒 ({[round(_, 3) for _ in All_total_train_time]})")
