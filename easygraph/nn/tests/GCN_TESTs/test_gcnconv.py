...
def Show_Result(dataset_name, RR, All_total_train_time, All_epoch_times, All_forward_times, All_backward_times, peak_memory_mb, All_test_acc, ALL_f1):
    ...
    print(f"🔥 总训练时间: {sum(All_total_train_time)/RR:.3f} 秒 ({[round(_, 3) for _ in All_total_train_time]})")
    print(f"🔥 总训练时间标准差: {statistics.stdev(All_total_train_time):.3f} 秒")
...
def main():
    ...
    for backend_type, dataset_name in DATASETS:
        ...
        for R in tqdm(range(RR + 1)):
            ...
            if 'adj_gp' in g.cache:
                del g.cache['adj_gp']  # 删除 adj_gp
            g.build_adj_gp(nparts=32) # 重新分割

            # 3️⃣ 获取新的 perm 并应用到数据和 mask 上
            perm = g.cache['gp_perm']
            # 每次循环用原始数据生成 perm 后的新数据
            data.x = x_orig[perm]
            data.y = y_orig[perm]
...
if __name__ == "__main__":
    ...
    df.to_csv("gcn_easygraph_results.csv")