...
# -------------------- 包和数据导入 --------------------
from torch_geometric.datasets import Coauthor, Planetoid, Yelp, Reddit, Flickr, Amazon
...
for backend_type, dataset_name in DATASETS:
    ...
    elif backend_type =='ogb2':
        dataset = PygNodePropPredDataset(name=dataset_name, root='/root/autodl-tmp/zss/data/OGB')
    ...
    elif backend_type in ['Reddit', 'Flickr', 'Coauthor']: 
    ...
    for R in tqdm(range(RR)):
        ...
        for epoch in range(1, EPOCHS+1):
            ...
            # -------------------- 子图 loss --------------------
            loss = criterion(out[train_mask_sub], data.y[sampled_nodes_tensor][train_mask_sub].squeeze())
            # loss_val = criterion(out[val_mask_sub], data.y[sampled_nodes_tensor][val_mask_sub].squeeze())
            # loss_test = criterion(out[test_mask_sub], data.y[sampled_nodes_tensor][test_mask_sub].squeeze())

            # -------------------- 全图验证 --------------------
            model.eval()
            with torch.no_grad():
                out_full = model(data.x, g)  # 全图前向
                loss_val = criterion(out_full[data.val_mask], data.y[data.val_mask].squeeze())
