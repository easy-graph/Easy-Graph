...
from cogdl.models.nn.gat import GAT
...
for backend_type, dataset_name in DATASETS:
    ...
    for R in tqdm(range(RR)):
        # -------------------- 初始化模型 --------------------
        model = GAT(in_feats = dataset.num_node_features, hidden_size = HIDDEN_DIM, 
                    out_features = dataset.num_classes, num_layers = 2, dropout=DROPOUT, alpha=0.2,
                    attn_drop = DROPOUT, nhead= Heads, residual=False, last_nhead=1, norm=None).to(DEVICE)
