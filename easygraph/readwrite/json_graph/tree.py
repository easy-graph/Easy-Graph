...
def tree_data(G, root, ident="id", children="children"):
    ...
    ident : string
        Attribute name for storing EasyGraph-internal graph data. `ident` must
        have a different value than `children`. The default is 'id'.

    children : string
        Attribute name for storing EasyGraph-internal graph data. `children`
    ...
    if not eg.is_weakly_connected(G):
        raise TypeError("G is not weakly connected.")

    if ident == children:
        raise eg.EasyGraphError("The values for `id` and `children` must be different.")
